...
# Train an L2-regularized Primal L1-SVM classifier
...
w = Variable(d)
loss = sum_entries(pos(1 - mul_elemwise(Y, X*w)))
reg = 0.5 * sum_squares(w)
C = 4.0
prob = Problem(Minimize(C * loss + reg))
elapsed = timeit.default_timer() - start_time
print "%f seconds to formulate L1-SVM with L2 regularization" % (elapsed)
